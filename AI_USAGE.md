...
### T-037 — Ticket 34 · Victory confetti effect on Level Complete (with proper cleanup)

- **Task / problem addressed:** When the player clears a level, a confetti burst plays automatically the moment the Level Complete UI renders (`_VictoriaOverlay`, i.e. `JuegoViewState.victoria != null`) — and **not** on defeat, pause, or mere rebuilds of an already-shown victory state (fires **once per win**). It must play on **bonus** wins too (`mostrarPuntuacion == false`), stay purely visual/non-blocking (never intercept or delay the Next/Retry/Menu buttons), and **clean up on unmount** (dispose its controller / cancel timers) so leaving mid-burst leaks no ticker. Scope defined by `.issues/34-feat-frontend-victory-confetti.md`. The ticket also mandated a **dependency review first**: the project ships no animation/particle package, so a self-contained Flutter `CustomPainter` + `AnimationController` was preferred over adding one.
- **AI tool used:** Claude Code (Opus 4.8 / claude-opus-4-8).
- **Prompt / instruction:** (verbatim) "please implement this ticket `c:\Users\maria\Desktop\ArrowMaze\arrowmaze-frontend\.issues\34-feat-frontend-victory-confetti.md` following the rules in these skills `c:\Users\maria\Desktop\ArrowMaze\arrowmaze-frontend\.claude\skills\clean-architecture` `c:\Users\maria\Desktop\ArrowMaze\arrowmaze-frontend\.claude\skills\tdd-strict`".
- **Result obtained:** Strict TDD (red → green → refactor). **Dependency decision:** no new package — `canvas-confetti` is JS/web-only (not a Flutter option), and adding `confetti` was unnecessary given the existing hand-rolled-painter idiom (`_RelojHud`, `_SalidaPainter`); **zero new deps**. Produced: `lib/presentation/views/game/confetti_overlay.dart` — `ConfettiOverlay`, a **presentation-only** `StatefulWidget` (imports only `dart:math` + `flutter/material`; no ViewModel/domain symbols) that owns one `AnimationController` via `SingleTickerProviderStateMixin`, **fires once** in `initState` (never restarts on rebuild), paints a projectile-motion particle field with a dumb `_ConfettiPainter` (position/fade are pure functions of `t`; particles seeded once from a fixed seed — no per-frame allocation), fades out over the final third, is wrapped in `IgnorePointer` (AC5), and disposes the controller in `dispose`; particle params (count/duration/gravity) are named, overridable constants (♻️ refactor). Wiring in `lib/presentation/views/game/game_view.dart`: added an optional full-bleed `fondo` layer to `_Overlay` (drawn behind the centered panel content, above the scrim); `_VictoriaOverlay` passes `fondo: ConfettiOverlay(colores: game.arrowPalette)` so confetti sits under the win text/stars, tinted to match the board — and, because this overlay renders for **every** victory, it covers bonus wins for free (AC3). New tests: `test/presentation/confetti_overlay_test.dart` (`should_start_animation_when_first_mounted` via `tester.hasRunningAnimations` — AC1; `should_fire_only_once_across_rebuilds` — AC2; `should_dispose_controller_on_unmount` leak-check — AC4) and `test/presentation/game_view_test.dart` (`should_show_confetti_when_victoria_present_and_not_on_derrota` — asserts absent while playing, present on win, and absent on a move-budget defeat; `should_show_confetti_on_bonus_victory` — AC3). Verified: full suite **439 green** (434 prior + 5 new); `flutter analyze` clean (no errors/warnings on the new files; only the pre-existing info-level `prefer_initializing_formals` hints remain).
